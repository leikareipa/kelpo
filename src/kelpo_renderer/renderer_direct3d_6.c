...
{
    if (!kelpo_window__create_window(screenWidth, screenHeight, RENDERER_NAME, kelpo_surface_direct3d_6__window_message_handler))
    {
        goto initialization_failed;
    }

    if (!kelpo_surface_direct3d_6__create_surface(screenWidth, screenHeight, screenBPP, 1, deviceId))
    {
        kelpo_surface_direct3d_6__release_surface();
        goto initialization_failed;
    }

    if (!kelpo_rasterizer_direct3d_6__initialize())
    {
        kelpo_rasterizer_direct3d_6__release();
        kelpo_surface_direct3d_6__release_surface();
        goto initialization_failed;
    }
...
{
    return (kelpo_surface_direct3d_6__release_surface() &
            kelpo_rasterizer_direct3d_6__release() &
            kelpo_window__release_window());
}
...
int export_interface(struct kelpo_interface_s *const interface)
{
    ...
    interface->rasterizer.unload_textures = kelpo_rasterizer_direct3d_6__unload_textures;
