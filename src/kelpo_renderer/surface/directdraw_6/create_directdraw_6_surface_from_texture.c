...
extern LPDIRECTDRAW DIRECTDRAW_6;
                                                                   ...
                                                                   LPDIRECT3DDEVICE2 d3dDevice)
{
    ...
    assert(DIRECTDRAW_6 && "The DirectDraw 5 interface has not been initialized.");
    ...
    /* Create the texture's DirectDraw surface.*/
    {
        ...
        if (FAILED(hr = IDirectDraw2_CreateSurface(DIRECTDRAW_6, &surfaceDescription, &d3dTexture, NULL)))
        {
            fprintf(stderr, "DirectDraw 6: Failed to create a DirectDraw surface for a texture (error 0x%x).\n", hr);
            return NULL;
        }
    }
    ...
    for (m = 0; m < texture->numMipLevels; m++)
    {
        ...
        /* Copy the texture's mip level pixel data into the DirectDraw surface.*/
        {
            ...
            {
                fprintf(stderr, "DirectDraw 6: Failed to lock a texture surface (error 0x%x).\n", hr);
                return NULL;
            }

            assert(((mipSurfaceDesc.dwWidth == mipLevelSideLength) &&
                    (mipSurfaceDesc.dwHeight == mipLevelSideLength)) &&
                    "DirectDraw 6: Invalid texture surface dimensions.");
            ...
            assert(((mipSurfaceDesc.ddpfPixelFormat.dwRGBAlphaBitMask == 0x8000) &&
                    ...
                    "DirectDraw 6: Invalid pixel format for a texture surface. Expected ARGB 1555.");
            ...
            {
                fprintf(stderr, "DirectDraw 6: Failed to unlock a texture surface (error 0x%x).\n", hr);
                return NULL;
            }
        }
